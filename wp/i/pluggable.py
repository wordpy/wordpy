...
# These functions can be replaced via plugins. If plugins do not redefine
...
if ( !function_exists('wp_set_current_user') ) :
  ...
  Set $id to None and specify a name if you do not know a user's ID.
...
#UserObj or UserObjs = get_user_by(D.SDB.DbConn,'user_login','wordpy_info')
def get_user_by(field, value):
  ...
  @return WP_User|False WP_User object on success, False on failure.
  '''
  userdata = WcU.WP_User.get_data_by( field, value )
  if not userdata:
    return False
  user = WcU.WP_User()
  user.Init(userdata)
...
function wp_mail( $to, $subject, $message, $headers = '', $attachments = array() ) {
  ...
  $atts = apply_filters( 'wp_mail', compact(locals(), 'to', 'subject', 'message', 'headers', 'attachments' ) )
  ...
  $address_headers = compact(locals(), 'to', 'cc', 'bcc', 'reply_to' )
  ...
  try {
    ...
    $mail_error_data = compact(locals(), 'to', 'subject', 'message', 'headers', 'attachments' )
...
function wp_authenticate($username, $password) {
  ...
  # WP_Error or None otherwise.
   *
  # @since 2.8.0
  # @since 4.5.0 `$username` now accepts an email address.
   *
  # @param None|WP_User|WP_Error $user     WP_User if the user is authenticated.
  #                                        WP_Error or None otherwise.
  ...
  if (WpC.WB.Wj.is_wp_error($user) && !in_array($user->get_error_code(), $ignore_codes) ) {
    /**
...
function wp_validate_auth_cookie($cookie = '', $scheme = '') {
  ...
  if ( $expiration < time() ) {
    ...
    WpC.WB.Wj.login_grace_period = 1
  }
...
function wp_parse_auth_cookie($cookie = '', $scheme = '') {
  ...
  return compact(locals(), 'username', 'expiration', 'token', 'hmac', 'scheme' )
}
endif
...
function check_admin_referer( $action = -1, $query_arg = '_wpnonce' ) {
  if ( -1 == $action )
    WiFc._doing_it_wrong( function_exists.__name__,    # __FUNCTION__,
...
function check_ajax_referer( $action = -1, $query_arg = False, $die = True ) {
  if -1 == $action:
    WiFc._doing_it_wrong( .__name__,    # __FUNCTION__,
...
if ( ! function_exists('wp_notify_postauthor') ) :
  ...
  '''
function wp_notify_postauthor( $comment_id, $deprecated = None ) {
  if ( None !== $deprecated ) {
...
if ( !function_exists('wp_new_user_notification') ) :
  ...
  @param int    $user_id    User ID.
  @param None   $deprecated Not used (argument deprecated).
  @param string $notify     Optional. Type of notification that should happen. Accepts 'admin' or an empty
                            string (admin only), 'user', or 'both' (admin and user). Default empty.
  '''
function wp_new_user_notification( $user_id, $deprecated = None, $notify = '' ) {
  if ( $deprecated !== None ) {
...
# checkout!!! github.com/morissette/insecure-wp-admin-password-check
# checkout!!! mattharris.org/2014/10/python-authentication-wordpress/
# checkout!!! github.com/exavolt/python-phpass

def wp_hash_password( password):
  ...
  return phpass.encrypt( password.strip() )
...
if ( !function_exists( 'get_avatar' ) ) :
  ...
  '''
function get_avatar( $id_or_email, $size = 96, $default = '', $alt = '', $args = None ) {
  $defaults = array(
   ...
   *
  # Passing a non-None value will effectively short-circuit get_avatar(), passing
  # the value through the {@see 'get_avatar'} filter and returning early.
   *
  # @since 4.2.0
   *
  # @param string $avatar      HTML for the user's avatar. Default None.
  # @param mixed  $id_or_email The Gravatar to retrieve. Accepts a user_id, gravatar md5 hash,
  #                            user email, WP_User object, WP_Post object, or WP_Comment object.
  ...
  if ( ! $url || WpC.WB.Wj.is_wp_error( $url ) ) {
    return False
  }
...
if ( !function_exists( 'wp_text_diff' ) ) :
  ...
  '''
function wp_text_diff( $left_string, $right_string, $args = None ) {
