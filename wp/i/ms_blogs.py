...
function get_blog_details( $fields = null, $get_all = true ) {
	...
	if ( is_array($fields ) ) {
		...
		if ( ! $fields )
			$blog_id = Wj.get_current_blog_id();
...
function get_blog_option( $id, $option, $default = false ) {
	...
	if ( empty( $id ) )
		$id = Wj.get_current_blog_id();

	if ( Wj.get_current_blog_id() == $id )
...
function switch_to_blog( $new_blog, $deprecated = null ) {
	...
	WpC.WB.Wj._wp_switched_stack[] = $blog_id;
	...
	if ( $new_blog == $blog_id ) {
		...
		WpC.WB.Wj.switched = true;
		return true;
	}

	$wpdb->set_blog_id( $new_blog );
	#GLOBALS['table_prefix'] = $wpdb->get_blog_prefix();
	WpC.WB.Wj.table_prefix = $wpdb->get_blog_prefix();
	$prev_blog_id = $blog_id;
	#GLOBALS['blog_id'] = $new_blog;
	WpC.WB.Wj.blog_id = $new_blog;
...
function ms_is_switched() {
	#return not Php.empty( $GLOBALS['_wp_switched_stack'] );
	return not Php.empty( WpC.WB.Wj.__dict__, '_wp_switched_stack' );
}
