...
def get_default_comment_status( post_type = 'post', comment_type = 'comment' ):
  ...
  if 'page' == post_type:
    status = 'closed'
  elif WiP.post_type_supports( post_type, supports ):
    status = WiO.get_option( "default_"+ option +"_status" )
  else:
    status = 'closed'
...
function sanitize_comment_cookies() {
  if ( isset( $_COOKIE['comment_author_' . COOKIEHASH] ) ) {
    ...
    $comment_author = WiF.wp_unslash($comment_author)
  ...
  if ( isset( $_COOKIE['comment_author_email_' . COOKIEHASH] ) ) {
    ...
    $comment_author_email = WiF.wp_unslash($comment_author_email)
  ...
  if ( isset( $_COOKIE['comment_author_url_' . COOKIEHASH] ) ) {
    ...
    $comment_author_url = WiF.wp_unslash($comment_author_url)
...
function wp_allow_comment( $commentdata, $avoid_die = false ) {
  ...
  $dupe = $wpdb->prepare(
    ...
    WiF.wp_unslash( $commentdata['comment_post_ID'] ),
    WiF.wp_unslash( $commentdata['comment_parent'] ),
    WiF.wp_unslash( $commentdata['comment_author'] )
  )
  if ( $commentdata['comment_author_email'] ) {
    $dupe .= $wpdb->prepare(
      "AND comment_author_email = %s ",
      WiF.wp_unslash( $commentdata['comment_author_email'] )
    )
  }
  $dupe .= $wpdb->prepare(
    ") AND comment_content = %s LIMIT 1",
    WiF.wp_unslash( $commentdata['comment_content'] )
  )
...
function wp_get_comment_fields_max_lengths() {
  ...
  if ( $wpdb->is_mysql ) {
    foreach ( $lengths as $column => $length ) {
      ...
      if ( WpC.WB.Wj.is_wp_error( $col_length ) ) {
        break
      }
...
function wp_get_current_commenter() {
  ...
  # }
  return apply_filters( 'wp_get_current_commenter', compact(locals(),'comment_author', 'comment_author_email', 'comment_author_url') )
}
...
function wp_insert_comment( $commentdata ) {
  wpdb = WpC.WB.Wj.wpdb  # global wpdb
  $data = WiF.wp_unslash( $commentdata )
  ...
  $compacted = compact(locals(), 'comment_post_ID', 'comment_author', 'comment_author_email', 'comment_author_url', 'comment_author_IP', 'comment_date', 'comment_date_gmt', 'comment_content', 'comment_karma', 'comment_approved', 'comment_agent', 'comment_type', 'comment_parent', 'user_id' )
...
function wp_update_comment($commentarr) {
  ...
  $data = WiF.wp_unslash( $commentarr )
  ...
  $rval = $wpdb->update( $wpdb->comments, $data, compact(locals(), 'comment_ID' ) )
...
function discover_pingback_server_uri( $url, $deprecated = '' ) {
  ...
  if ( WpC.WB.Wj.is_wp_error( $response ) )
    return false
